...
def train(Y, loss_fn):
    ...
    for it in range(n_iterations):
        ...
        for i in range(n_steps):
            obj = lf_rho(x, lmbda, mu, rho, loss_fn=loss_fn)
        ...
        objs.append([lf_rho(x,lmbda, mu, rho, loss_fn=loss_fn).item(), lf(x, lmbda, mu, loss_fn=loss_fn).item(), f(x, loss_fn=loss_fn).item()])    
...
df_weights = pd.DataFrame(
    np.around(100*np.array(weights).T,2),
    index=data_loader.df.columns,
    columns=loss_fns).iloc[::-1]
